...
ggplot(data=autos, aes(x=length, y=width))+
 geom_point(aes(size=height),alpha=0.1)+
...
```{r}
ggplot(df_country, aes(x=fecha, y=value))+geom_point(size=0.2)+geom_line(aes(color=Country.Region))
```
...
df_country |> mutate(year=year(df_country$fecha)) |> group_by(year, country=Country.Region) |> summarise(value=max(value)) -> df_country_year

ggplot(df_country_year, aes(x=year, y=value, color=country, fill=country, group=country)) +
  geom_col(position = 'dodge')+ylab('Número de casos')+xlab('Año')+
  geom_label(aes(label=value), fill='white', color='black',
